...
namespace NameSpace_Function::NameSpace_Render::NameSpace_Pass {
	...
	Combine_UI_Pass::Combine_UI_Pass(const Render_Pass_Command_Info& Command_Info) :
	...
	void Combine_UI_Pass::Setup_Descriptor_Set_Layout(void) {
		...
		RHI_Descriptor_Set_Layout_Binding Global_Layout_Scene_Input_Attachment_Binding{};
		{
			Global_Layout_Scene_Input_Attachment_Binding.Binding = 0;
			Global_Layout_Scene_Input_Attachment_Binding.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
			Global_Layout_Scene_Input_Attachment_Binding.Descriptor_Count = 1;
			Global_Layout_Scene_Input_Attachment_Binding.Stage_Flags = to_underlying(RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_FRAGMENT_BIT);
			Global_Layout_Scene_Input_Attachment_Binding.Immutable_Samplers = nullptr;
		}

		RHI_Descriptor_Set_Layout_Binding Global_Layout_Normal_Input_Attachment_Binding{};
		{
			Global_Layout_Normal_Input_Attachment_Binding.Binding = 1;
			Global_Layout_Normal_Input_Attachment_Binding.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
			Global_Layout_Normal_Input_Attachment_Binding.Descriptor_Count = 1;
			Global_Layout_Normal_Input_Attachment_Binding.Stage_Flags = to_underlying(RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_FRAGMENT_BIT);
			Global_Layout_Normal_Input_Attachment_Binding.Immutable_Samplers = nullptr;
		}

		const vector<const RHI_Descriptor_Set_Layout_Binding*> Bindings{
			&Global_Layout_Scene_Input_Attachment_Binding,
	...
	void Combine_UI_Pass::Setup_Descriptor_Set(void) {
	...
	void Combine_UI_Pass::Setup_Pipeline(void) {
		...
		unique_ptr<RHI_Shader_Module> Fragment_Shader_Module{ this->m_RHI->Create_Shader_Module(&COMBINE_UI_FRAG) };
		...
		RHI_Graphics_Pipeline_Create_Info Graphics_Pipeline_Create_Info{};
		{
			...
			Graphics_Pipeline_Create_Info.Subpass = _main_camera_subpass_combine_ui;
	...
	void Combine_UI_Pass::Draw(void) {
	...
	void Combine_UI_Pass::Pre_Inittialize(shared_ptr<Render_Pass_Pre_Initialize_Info> Init_Info) {
		const auto Combine_UI_Info{ static_pointer_cast<Combine_UI_Render_Pass_Pre_Initialize_Info>(Init_Info) };
		{
			...
			this->m_UI_Input_attachment = Combine_UI_Info->UI_Input_attachment;
		}
