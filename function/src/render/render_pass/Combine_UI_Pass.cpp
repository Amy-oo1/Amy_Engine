...
namespace NameSpace_Function::NameSpace_Render::NameSpace_Pass {
	...
	Combine_UI_Pass::Combine_UI_Pass(const Render_Pass_Command_Info& Command_Info, RHI_Render_Pass* Render_Pass) :
	...
	void Combine_UI_Pass::Updata_After_Frame_Buffer_ReCreate(RHI_Image_View* Scene_Inupt, RHI_Image_View* UI_Inupt) {
		RHI_Descriptor_Image_Info Scene_Image_Info{};
		{
			Scene_Image_Info.Sampler = this->m_RHI->Get_Default_Sampler(RHI_DEFAULT_SAMPLER_TYPE::DEFAULT_SAMPLER_NEAREST);
			Scene_Image_Info.Image_View = Scene_Inupt;
			Scene_Image_Info.Image_Layout = RHI_IMAGE_LAYOUT::RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		}
		RHI_Descriptor_Image_Info UI_Image_Info{};
		{
			UI_Image_Info.Sampler = this->m_RHI->Get_Default_Sampler(RHI_DEFAULT_SAMPLER_TYPE::DEFAULT_SAMPLER_NEAREST);
			UI_Image_Info.Image_View = UI_Inupt;
			UI_Image_Info.Image_Layout = RHI_IMAGE_LAYOUT::RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		}
		...
		RHI_Write_Descriptor_Set Image_Write{};
		{
			Image_Write.sType = RHI_STRUCT_TYPE::RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			Image_Write.Dst_Set = this->m_Descriptors[0].Descriptor_Set.get();
			Image_Write.Dst_Binding = 0;
			Image_Write.Dst_Array_Element = 0;
			Image_Write.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
			Image_Write.Image_Infos = &Image_Infos;
		}
		const vector<const RHI_Write_Descriptor_Set*> Writes{ &Image_Write };
