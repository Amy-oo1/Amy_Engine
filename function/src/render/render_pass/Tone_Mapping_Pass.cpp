...
namespace NameSpace_Function::NameSpace_Render::NameSpace_Pass {
	...
	Tone_Mapping_Pass::Tone_Mapping_Pass(const Render_Pass_Command_Info& Command_Info) :
		Render_Pass{ Command_Info } {
	}

	void Tone_Mapping_Pass::Update_After_Frame_Buffer_ReCreate(RHI_Image_View* Input_Attachment) {
		...
		const vector<const RHI_Descriptor_Image_Info*> Post_Process_Per_Fame_Iput_Attachment_Infos{ &Post_Process_Per_Fame_Input_Attachment_Info };

		RHI_Write_Descriptor_Set Post_Process_Descriptor_Write_Set{};
		{
			Post_Process_Descriptor_Write_Set.sType = RHI_STRUCT_TYPE::RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			Post_Process_Descriptor_Write_Set.Dst_Set = this->m_Descriptors[0].Descriptor_Set.get();
			Post_Process_Descriptor_Write_Set.Dst_Binding = 0;
			Post_Process_Descriptor_Write_Set.Dst_Array_Element = 0;
			Post_Process_Descriptor_Write_Set.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
			Post_Process_Descriptor_Write_Set.Image_Infos = &Post_Process_Per_Fame_Iput_Attachment_Infos;
		}

		const vector<const RHI_Write_Descriptor_Set*> Writes{ &Post_Process_Descriptor_Write_Set };
	...
	void Tone_Mapping_Pass::Setup_Descriptor_Set_Layout(void) {
		...
		const vector<const RHI_Descriptor_Set_Layout_Binding*> Post_Process_Global_Layout_Bindings{ &Post_Process_Global_Layout_Input_Attachment_Binding };
		...
		RHI_Descriptor_Set_Layout_Create_Info Descriptor_Set_Layout_Create_Info{};
		{
			...
			Descriptor_Set_Layout_Create_Info.Bindings = &Post_Process_Global_Layout_Bindings;
		}
	...
	void Tone_Mapping_Pass::Setup_Pipeline(void) {
		...
		unique_ptr<RHI_Shader_Module> Fragment_Shader_Module{ this->m_RHI->Create_Shader_Module(&TONE_MAPPING_FRAG) };
		...
		RHI_Graphics_Pipeline_Create_Info Graphics_Pipeline_Create_Info{};
		{
			...
			Graphics_Pipeline_Create_Info.Subpass = _main_camera_subpass_tone_mapping;
