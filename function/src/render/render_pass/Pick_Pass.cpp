...
namespace NameSpace_Function::NameSpace_Render::NameSpace_Pass {
	...
	using NameSpace_Render_System::Mesh_Inefficient_Pick_Per_Draw_Call_Storage_Buffer_Object;
	using NameSpace_Render_System::Mesh_Inefficient_Pick_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Object;

	using NameSpace_Render_System::Mesh_Vertex;

	void Pick_Pass::Pre_Inittialize(const Render_Pass_Pre_Initialize_Info* Init_Info) {
		const auto Pick_Info{ static_cast<const Pick_Render_Pass_Pre_Initialize_Info*>(Init_Info) };
		{
			this->m_Per_Mesh_Set_Layout = Pick_Info->Per_Mesh_Set_Layout;
		}
	...
	void Pick_Pass::Post_Inittialize(const Render_Pass_Post_Initialize_Info* Init_Info)
	{
	}
	...
	void Pick_Pass::Setup_Attachments(void) {
		...
		Ref_Attachments.resize(1);

		Ref_Attachments[0].Format = RHI_FORMAT::RHI_FORMAT_R32_UINT;
		...
		std::tie(Ref_Attachments[0].Image, Ref_Attachments[0].Image_Memory) = this->m_RHI->Create_Image(
			...
			RHI_IMAGE_USAGE_FLAG_BITS::RHI_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | RHI_IMAGE_USAGE_FLAG_BITS::RHI_IMAGE_USAGE_TRANSFER_SRC_BIT,
	...
	void Pick_Pass::Setup_Render_Pass(void) {
		RHI_Attachment_Description Color_Attachment_Description{};
		{
			...
			Color_Attachment_Description.Initial_Layout = RHI_IMAGE_LAYOUT::RHI_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
			Color_Attachment_Description.Final_Layout = RHI_IMAGE_LAYOUT::RHI_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
		}
	...
	void Pick_Pass::Setup_Frame_Buffer(void) {
	...
	void Pick_Pass::Setup_Descriptor_Set_Layout(void) {
		this->m_Descriptors.resize(1);

		RHI_Descriptor_Set_Layout_Binding Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Binding{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Binding.Binding = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Binding.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Binding.Descriptor_Count = 1;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Binding.Stage_Flags = to_underlying(RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_VERTEX_BIT);
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Binding.Immutable_Samplers = nullptr;
		}

		RHI_Descriptor_Set_Layout_Binding Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Binding{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Binding.Binding = 1;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Binding.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Binding.Descriptor_Count = 1;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Binding.Stage_Flags = to_underlying(RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_VERTEX_BIT);
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Binding.Immutable_Samplers = nullptr;
		}

		RHI_Descriptor_Set_Layout_Binding Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding.Binding = 2;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding.Descriptor_Count = 1;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding.Stage_Flags = to_underlying(RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_VERTEX_BIT);
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding.Immutable_Samplers = nullptr;
		}

		const vector<const RHI_Descriptor_Set_Layout_Binding*> Bindings{
			&Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Binding,
			&Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Binding,
			&Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding
		};
	...
	void Pick_Pass::Setup_Descriptor_Set(void) {
		...
		RHI_Descriptor_Buffer_Info Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Info{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Info.Buffer = this->m_Global_Render_Resource->Storage_Buffer.Global_Upload_Ring_Buffer.get();
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Info.Offset = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Info.Range = sizeof(Mesh_Inefficient_Pick_Per_Frame_Storage_Buffer_Object);
		}
		const vector<const RHI_Descriptor_Buffer_Info*> Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Infos{ &Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Info };

		RHI_Descriptor_Buffer_Info Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Info{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Info.Buffer = this->m_Global_Render_Resource->Storage_Buffer.Global_Upload_Ring_Buffer.get();
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Info.Offset = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Info.Range = sizeof(Mesh_Inefficient_Pick_Per_Draw_Call_Storage_Buffer_Object);
		}
		const vector<const RHI_Descriptor_Buffer_Info*> Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Infos{ &Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Info };

		RHI_Descriptor_Buffer_Info Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Info{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Info.Buffer = this->m_Global_Render_Resource->Storage_Buffer.Global_Upload_Ring_Buffer.get();
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Info.Offset = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Info.Range = sizeof(Mesh_Inefficient_Pick_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Object);
		}
		const vector<const RHI_Descriptor_Buffer_Info*> Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Infos{ &Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Info };

		RHI_Write_Descriptor_Set Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write.sType = RHI_STRUCT_TYPE::RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write.Dst_Set = this->m_Descriptors[0].Descriptor_Set.get();
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write.Dst_Binding = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write.Dst_Array_Element = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
			Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write.Buffer_Infos = &Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Infos;
		}

		RHI_Write_Descriptor_Set Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write.sType = RHI_STRUCT_TYPE::RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write.Dst_Set = this->m_Descriptors[0].Descriptor_Set.get();
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write.Dst_Binding = 1;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write.Dst_Array_Element = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write.Buffer_Infos = &Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Infos;
		}

		RHI_Write_Descriptor_Set Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write{};
		{
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write.sType = RHI_STRUCT_TYPE::RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write.Dst_Set = this->m_Descriptors[0].Descriptor_Set.get();
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write.Dst_Binding = 2;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write.Dst_Array_Element = 0;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
			Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write.Buffer_Infos = &Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Infos;
		}

		const vector<const RHI_Write_Descriptor_Set*> Writes{
			&Mesh_Inefficient_Pick_Global_Layout_Per_Frame_Storage_Buffer_Write,
			&Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Storage_Buffer_Write,
			&Mesh_Inefficient_Pick_Global_Layout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Write
		};

		this->m_RHI->Update_Descriptor_Sets(&Writes, nullptr);
	}

	void Pick_Pass::Setup_Pipeline(void) {
		...
		unique_ptr<RHI_Shader_Module> Vertex_Shader_Module{ this->m_RHI->Create_Shader_Module(&MESH_INEFFICIENT_PICK_VERT) };
		...
		unique_ptr<RHI_Shader_Module> Fragment_Shader_Module{ this->m_RHI->Create_Shader_Module(&MESH_INEFFICIENT_PICK_FRAG) };
