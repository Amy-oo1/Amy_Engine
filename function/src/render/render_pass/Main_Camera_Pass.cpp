...
namespace NameSpace_Function::NameSpace_Render::NameSpace_Pass {
	...
	void Main_Camera_Pass::Set_Driectional_Light_Shadow_Color_Image_View(RHI_Image_View* Image_View) {
		this->m_Directional_Light_Shadow_Color_Image_View = Image_View;
	}
	...
	void Main_Camera_Pass::Setup_Frame_Buffer(void)
	{
	}
	...
	void Main_Camera_Pass::Setup_Descriptor_Set_Layout(void) {
...

		{
			RHI_Descriptor_Set_Layout_Binding Mesh_Global_Layout_Per_Frame_Storage_Buffer_Binding{};
			{
				...
				Mesh_Global_Layout_Per_Frame_Storage_Buffer_Binding.Stage_Flags = to_underlying(RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_VERTEX_BIT);
			...
			RHI_Descriptor_Set_Layout_Binding Mesh_Global_Lyout_Per_Draw_Call_Storage_Buffer_Binding{};
			{
				...
				Mesh_Global_Lyout_Per_Draw_Call_Storage_Buffer_Binding.Stage_Flags = RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_VERTEX_BIT | RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_FRAGMENT_BIT;
			...
			RHI_Descriptor_Set_Layout_Binding Mesh_Global_Lyout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding{};
			{
				...
				Mesh_Global_Lyout_Per_Draw_Call_Vertex_Blending_Storage_Buffer_Binding.Stage_Flags = RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_VERTEX_BIT | RHI_SHADER_STAGE_FLAG_BITS::RHI_SHADER_STAGE_FRAGMENT_BIT;
...

		{
			...
			{
				GBuffer_Depth_Emissive_Global_Layout_Input_Attachment_Binding.Binding = 2;
		...
		//NOTE :Mesh GBuffer
		{
			...
			RHI_Pipeline_Color_Blend_State_Create_Info Color_Blend_State_Create_Info{};
			{
				RHI_Pipeline_Color_Blend_Attachment_State Color_Blend_Attachment{};
				{
					...
					Color_Blend_Attachment.Dst_Color_Blend_Factor = NameSpace_RHI::RHI_BLEND_FACTOR::RHI_BLEND_FACTOR_ONE;
					...
					Color_Blend_Attachment.Dst_Alpha_Blend_Factor = NameSpace_RHI::RHI_BLEND_FACTOR::RHI_BLEND_FACTOR_ONE;
	...
	void Main_Camera_Pass::Setup_Model_Global_Descriptor_Set(void) {
		...
		const vector<const RHI_Descriptor_Image_Info*> Prefiltered_Image_Infos{
		...
		RHI_Write_Descriptor_Set Prefiltered_Image_Write{};
		{
			Prefiltered_Image_Write.sType = RHI_STRUCT_TYPE::RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			Prefiltered_Image_Write.Dst_Set = this->m_Descriptors[_mesh_global].Descriptor_Set.get();
			Prefiltered_Image_Write.Dst_Binding = 5;
			Prefiltered_Image_Write.Dst_Array_Element = 0;
			Prefiltered_Image_Write.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
			Prefiltered_Image_Write.Image_Infos = &Prefiltered_Image_Infos;
		}
	...
	void Main_Camera_Pass::Setup_Skybox_Descriptor_Set(void) {
		...
		RHI_Write_Descriptor_Set Specular_Image_Write{};
		{
			...
			Specular_Image_Write.Dst_Binding = 0;
	...
	void Main_Camera_Pass::Setup_Axis_Descriptor_Set(void) {
		...
		RHI_Write_Descriptor_Set Axis_Stroage_Buffer_Write{};
		{
			...
			Axis_Stroage_Buffer_Write.Descriptor_Type = RHI_DESCRIPTOR_TYPE::RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
	...
	void Main_Camera_Pass::Setup_GBuffer_Light_Descriptor_Set(void) {
	...
	void Main_Camera_Pass::Setup_Swapchain_Frame_Buffers(void) {
		...
		for (uint32_t Index = 0; Index < this->m_RHI->Get_SwapChain_Image_Size(); ++Index) {
			...
			RHI_Frame_Buffer_Create_Info Frame_Buffer_Create_Info{};
			{
				...
				Frame_Buffer_Create_Info.Height = this->m_RHI->Get_SwapChain_Extent().Height;
	...
	void Main_Camera_Pass::Setup_Particle_Pass(void) {
		this->m_Particle_Pass->Set_Depth_and_Normal_Image(
			this->m_RHI->Get_SwapChain_Depth_Image(),
			this->m_Frame_Buffer.Attachments[_main_camera_pass_gbuffer_a].Image.get()
		);

		this->m_Particle_Pass->Set_Render_Pass_Handle(this->m_Frame_Buffer.Render_Pass.get());
	}
