...
namespace NameSpace_Core::NameSpace_Math {
	...
	bool Vector3::operator==(const Vector3& R_HS) const {
		return
	...
	float Vector3::Length_Square(void) const {
		return
			this->m_X * this->m_X +
			this->m_Y * this->m_Y +
			this->m_Z * this->m_Z;
	...
	float Vector3::Distance(const Vector3& R_HS) const {
		return ((*this) - R_HS).Length();
	}

	float Vector3::Distance_Square(const Vector3& R_HS) const {
		return ((*this) - R_HS).Length_Square();
	}

	float Vector3::Dot_Product(const Vector3& R_HS) const {
		return
			this->m_X * R_HS.m_X +
			this->m_Y * R_HS.m_Y +
			this->m_Z * R_HS.m_Z;
	...
	float Vector3::Get_X(void) const {
		return this->m_X;
	}

	float Vector3::Get_Y(void) const {
		return this->m_Y;
	}

	float Vector3::Get_Z(void) const {
		return this->m_Z;
	}
