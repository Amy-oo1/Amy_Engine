...
namespace NameSpace_Render::NameSpace_RHI::NameSpace_Vulkan_RHI::NameSpace_Utilities {
	...
	const VkImageView Create_Image_View(
		...
		THROW_IF_VK_FAILED(vkCreateImageView(Device, &View_Info, pAllocator, &Image_View));
		return Image_View;
	}
	...
	void Create_Cube_Map(
		...
		VkDeviceMemory& Memory,
		const VkAllocationCallbacks* VK_Allocations) {
		VkDeviceSize Image_Byte_Size{ Get_Image_Byte_Size(Image_Extent.width, Image_Extent.height, Format) * 6 };
		...
		for (size_t Index = 0; Index < 6; ++Index) {
			memcpy(static_cast<unsigned char*>(Dst_Data) + Image_Byte_Size / 6 * Index, Image_Pixels[Index], static_cast<size_t>(Image_Byte_Size / 6));
		}
		vkUnmapMemory(Logical_Device, Staging_Memory);
